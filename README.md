...
sequenceDiagram
    ...
    Note over Alice: Генерирует RoutingToken_A<br/>(32 random bytes)
    Note over Bob: Генерирует RoutingToken_B<br/>(32 random bytes)
    
    Alice->>Server: Register(ListenTokens: [RoutingToken_A])
    Bob->>Server: Register(ListenTokens: [RoutingToken_B])
    
    Note over Server: Map:<br/>RoutingToken_A → Alice_Stream<br/>RoutingToken_B → Bob_Stream
    
    Note over Bob: Создает Invite<br/>RoutingToken = RoutingToken_B
    Bob->>Alice: Invite (содержит RoutingToken_B)
    
    Note over Alice: Сохраняет:<br/>OutboundRoutingToken = RoutingToken_B<br/>InboundRoutingToken = новый токен
    
    Alice->>Server: Message(routing_token: RoutingToken_B)
    Note over Server: Lookup RoutingToken_B<br/>→ Bob_Stream
...
### 9.4. Ключевые Изменения в Версии 0.2
